...
function Products({setCartQuantity}) {
  ...
  useEffect(()=>{
    ...
    async function fetchSearchedProducts(){
      // const response = await fetch(`https://dummyjson.com/products/search?q=${searchedProduct}?limit=15&skip=${(currPage - 1) * 15}`,
      const response = await fetch(`https://dummyjson.com/products/search?q=${searchedProduct}`,
        { method: "GET" }
      );
      const data = await response.json();
      if (data.products) {
